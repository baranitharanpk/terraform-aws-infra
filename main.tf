...
# CORRECTED VERSION - Creates inventory locally and outputs content
###################
...
apache-${idx + 1} ansible_host=${ip} ansible_user=ubuntu
%{~ endfor ~}

[nginx_servers]
%{~ for idx, ip in aws_instance.nginx[*].public_ip ~}
nginx-${idx + 1} ansible_host=${ip} ansible_user=ubuntu
%{~ endfor ~}
